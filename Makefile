...
k300: # OnLogic K300 target
	docker build --build-arg CONFIG_COMPRESSION=GZIP --build-arg CONFIG_PRIMARY=standard.json --build-arg CONFIG_MODULES=k300.txt -t registry2.lagoni.co.uk/os-builder:local .

magellis: # Schneider Magellis target
	docker build --build-arg CONFIG_COMPRESSION=GZIP --build-arg CONFIG_PRIMARY=standard.json --build-arg CONFIG_MODULES=magelis.txt -t registry2.lagoni.co.uk/oos-builder:local .
